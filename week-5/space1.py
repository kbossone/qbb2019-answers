...
while True:
    ...
    if nucl_id == 'query':
        nucl_seq = nucl_id.rstrip('/n')[1]
        nucleo[nucl_id] = nucl_seq
    else:
        nucl_id = nucl_id.rstrip('|')[3]
        nucl_seq = nucl_id.rstrip()[4]
        nucleo[nucl_id] = nucl_seq
...
for seq_ID in nucleo:
    if seq_ID not in back_to_nucleo:
        back_to_nucleo[seq_ID] = nucleo[seq_ID]
